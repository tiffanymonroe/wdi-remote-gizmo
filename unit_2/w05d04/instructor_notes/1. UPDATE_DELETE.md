...
3.  Next create a form to submit a delete request on the `index.js` page for each fruit.  (Does the form have to be inside or outside the loop?)
4.  Forms can't make delete requests on thier own so we need to use a node module.  Loop up `method-ovveride npm` the link to npmjs with the documentation of the module has a subsection with a header of `override using a query value` read that, and try to implement it.
5. Test it out
